...
<body>
    ...
    <c:if test="${post.id == 0}"><c:url var="actionUrl" value="/blog/write"/></c:if>
    <c:if test="${post.id != 0}"><c:url var="actionUrl" value="/blog/${post.id}/edit"/></c:if>
   <form:form action="${actionUrl}" 
        commandName="post" onsubmit="if($('#pen').html()!='<p><br></p>')$('#content').val($('#pen').html()); pen.destroy();" 
        method="post">
